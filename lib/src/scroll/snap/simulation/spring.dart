...
class ScrollSnapSpringSimulation extends Simulation {
  ...
  double _x(double time) => _c1 * math.pow(math.e, _r1 * time) + _c2 * math.pow(math.e, _r2 * time);

  @override
  double x(double time) => isDone(time) ? end : end + _x(time);

  @override
  double dx(double time) => _c1 * _r1 * math.pow(math.e, _r1 * time) + _c2 * _r2 * math.pow(math.e, _r2 * time);
  ...
  bool isDone(double time) {
    ...
    return nearZero(x, tolerance.distance) && nearZero(dx(time), tolerance.velocity);
  }

  @override
  String toString() => '${objectRuntimeType(this, 'SnapSpringSimulation')}(end: ${end.toStringAsFixed(1)})';
}
